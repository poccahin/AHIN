...
pub struct OkxReadonly;

#[async_trait]
impl ExchangeAdapter for OkxReadonly {
    ...
    async fn place_order_dry_run(&self, _order: OrderRequest) -> AppResult<OrderCandidate> {
        Err(AppError::ExecutionRejected(
            "OKX phase-one adapter cannot place or simulate orders".to_string(),
        ))
    }
}

fn readonly_stub() -> AppError {
    AppError::Exchange("OKX read-only adapter is not wired in phase one".to_string())
}
