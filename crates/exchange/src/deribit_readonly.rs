...
pub struct DeribitReadonly;

#[async_trait]
impl ExchangeAdapter for DeribitReadonly {
    ...
    async fn place_order_dry_run(&self, _order: OrderRequest) -> AppResult<OrderCandidate> {
        Err(AppError::ExecutionRejected(
            "Deribit phase-one adapter cannot place or simulate orders".to_string(),
        ))
    }
}

fn readonly_stub() -> AppError {
    AppError::Exchange("Deribit read-only adapter is not wired in phase one".to_string())
}
